...
namespace iCloud.Dav.Core.Services
{
    public class ConfigurableMessageHandler : DelegatingHandler
    {
        ...
        /// <summary>The User-Agent suffix header which contains the <see cref="ConfigurableMessageHandler.ApiVersion" />.</summary>
        ...
        /// <remarks>
        /// Since version 1.10, <see cref="ConfigurableMessageHandler.AddUnsuccessfulResponseHandler(IHttpUnsuccessfulResponseHandler)" /> and
        /// <see cref="ConfigurableMessageHandler.RemoveUnsuccessfulResponseHandler(IHttpUnsuccessfulResponseHandler)" /> were added in order to keep this class thread-safe.
        ...
        /// <remarks>
        /// Since version 1.10, <see cref="ConfigurableMessageHandler.AddExceptionHandler(IHttpExceptionHandler)" /> and <see cref="ConfigurableMessageHandler.RemoveExceptionHandler(IHttpExceptionHandler)" /> were added
        ...
        /// <remarks>
        /// Since version 1.10, <see cref="ConfigurableMessageHandler.AddExecuteInterceptor(IHttpExecuteInterceptor)" /> and <see cref="ConfigurableMessageHandler.RemoveExecuteInterceptor(IHttpExecuteInterceptor)" /> were
        ...
        /// <remarks>
        /// The number of allowed redirects (3xx) is defined by <see cref="ConfigurableMessageHandler.NumRedirects" />. This property defines
        /// only the allowed tries for &gt;=400 responses, or when an exception is thrown. For example if you set
        /// <see cref="ConfigurableMessageHandler.NumTries" /> to 1 and <see cref="ConfigurableMessageHandler.NumRedirects" /> to 5, the library will send up to five redirect
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var loggable = IsLoggingEnabled && ConfigurableMessageHandler.Logger.IsDebugEnabled;
            ...
            do
            {
                ...
                if (response == null)
                {
                    ...
                    if (loggable)
                        ConfigurableMessageHandler.Logger.Debug("Exception {0} was thrown, but it was handled by an exception handler", lastException.Message);
                }
                    ...
                    if (!flag1)
                    {
                        if (FollowRedirect && HandleRedirect(response))
                        {
                            if (redirectRemaining-- == 0)
                                triesRemaining = 0;
                            if (loggable)
                                ConfigurableMessageHandler.Logger.Debug("Redirect response was handled successfully. Redirect to {0}", response.Headers.Location);
                        }
                        else
                        {
                            if (loggable)
                                ConfigurableMessageHandler.Logger.Debug("An abnormal response wasn't handled. Status code is {0}", response.StatusCode);
                            triesRemaining = 0;
                        }
                    }
                    else if (loggable)
                        ConfigurableMessageHandler.Logger.Debug("An abnormal response was handled by an unsuccessful response handler. Status Code is {0}", response.StatusCode);
                }
            }
            ...
            if (!response.IsSuccessStatusCode)
                ConfigurableMessageHandler.Logger.Debug("Abnormal response is being returned. Status Code is {0}", response.StatusCode);
            return response;
        }
